...
Тебе всегда известны сегодняшняя дата и день недели. Если пользователь называет срок —
...
нужно постараться — интервалы, темповая, длинная). После — вызови set_plan_summary:
1-3 предложения логики плана отдельным абзацем, и отдельным абзацем, начинающимся с
"**Питание:**" — короткую рекомендацию по питанию под эту цель.
...
Явно скажи, что это новый план, вызови update_memory с новыми фактами, дальше веди его
самостоятельно (create_workout привяжет тренировки к новой активной цели).

Задачи:
1. Новый пользователь без цели — задай 3-5 вопросов (цель, уровень, ограничения, доступное
   время), затем в этом же ответе: update_memory → create_workout на весь срок → set_plan_summary.
...
const tools = [
  ...
  {
    name: 'create_workout',
    description: 'Добавить НОВУЮ тренировку в план — она сразу появится на экране "План". Не использовать для изменения уже существующей тренировки — для этого есть update_workout.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'update_workout',
    description: 'Изменить дату, описание и/или сложность уже существующей тренировки (перенос, корректировка нагрузки). Если не знаешь workout_id — не указывай, возьмётся ближайшая предстоящая тренировка пользователя.',
    input_schema: {
      type: 'object',
      properties: {
        workout_id: { type: 'number' },
        date: { type: 'string', description: 'Новая дата YYYY-MM-DD, если нужно перенести' },
        type: { type: 'string', description: 'Новое описание тренировки, если нужно изменить содержание' },
...
async function executeTool(userId, name, input) {
  ...
  if (name === 'create_workout') {
    ...
    const w = await db.query(
      `INSERT INTO workouts (goal_id, user_id, date, type, status, source, difficulty)
       VALUES ($1,$2,$3,$4,'planned','agent',$5) RETURNING id, date, type, difficulty`,
      [goalId, userId, input.date, input.type, input.difficulty || 'medium']
    );
  ...
  if (name === 'update_workout') {
    ...
    const r = await db.query(
      `UPDATE workouts SET date=COALESCE($1,date), type=COALESCE($2,type), difficulty=COALESCE($3,difficulty)
       WHERE id=$4 AND user_id=$5 RETURNING id, date, type, difficulty`,
      [input.date || null, input.type || null, input.difficulty || null, workoutId, userId]
    );
