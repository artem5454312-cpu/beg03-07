...
router.post('/profile-setup', requireAuth, async (req, res) => {
  const { name, photo_url, profession, gender, city, level } = req.body;
  await db.query(
  ...
  // Уровень подготовки выбран кнопками ещё в анкете — сразу кладём в память агента,
  // чтобы он не тратил на это отдельный вопрос и с первого сообщения знал тон и глубину плана.
  if (level) {
    await db.query(
      `INSERT INTO agent_memory (user_id, data, updated_at) VALUES ($1, $2, now())
       ON CONFLICT (user_id) DO UPDATE SET data = agent_memory.data || $2, updated_at = now()`,
      [req.userId, JSON.stringify({ level })]
    );
  }
  ...
  try {
    const kickoff = `(Служебная пометка, пользователь её не видел и не писал.) Это твоё первое обращение — ` +
      ...
      `${level ? `, уровень подготовки уже указал в анкете: "${level}" — не спрашивай про уровень заново, сразу калибруй тон и глубину плана под него` : ''}. ` +
      ...
      `(или два) из серии вводных — всего их будет до 6 подряд (цель, ограничения по здоровью, сколько ` +
      `дней/времени есть на тренировки и т.п.${level ? '' : ', уровень подготовки'}). Не задавай всё сразу, ` +
      `начни с одного-двух. В самом конце этого же сообщения (не как отдельный вопрос, просто короткой ` +
      `припиской) невзначай напомни: "Кстати, зайди в Профиль и включи уведомления — тогда я смогу писать ` +
      `тебе, даже если телефон заблокирован". Один раз, без занудства.`;
