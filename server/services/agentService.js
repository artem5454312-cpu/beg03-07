...
а не просто "силовая"). После — вызови set_plan_summary: 1-3 предложения логики плана
отдельным абзацем, и отдельным абзацем, начинающимся с "**Питание:**" — короткую
рекомендацию по питанию под эту цель.
...
const tools = [
  ...
  {
    name: 'update_workout',
    description: 'Изменить дату и/или описание уже существующей тренировки (перенос, корректировка нагрузки). Если не знаешь workout_id — не указывай, возьмётся ближайшая предстоящая тренировка пользователя.',
    input_schema: {
      type: 'object',
      properties: {
...
async function executeTool(userId, name, input) {
  ...
  if (name === 'update_workout') {
    ...
    const r = await db.query(
      `UPDATE workouts SET date=COALESCE($1,date), type=COALESCE($2,type)
       WHERE id=$3 AND user_id=$4 RETURNING id, date, type`,
      [input.date || null, input.type || null, workoutId, userId]
    );
