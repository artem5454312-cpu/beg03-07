...
router.post('/profile-setup', requireAuth, async (req, res) => {
  ...
  try {
    const kickoff = `(Служебная пометка, пользователь её не видел и не писал.) Это твоё первое обращение — ` +
      ...
      `Поздоровайся тепло, коротко представься как персональный AI-тренер, и задай первый вопрос (или два) ` +
      `из серии вводных вопросов — всего их будет до 6 подряд (цель, уровень подготовки, ограничения по ` +
      `здоровью, сколько дней/времени есть на тренировки и т.п.), чтобы понять цель и составить план. ` +
      `Не задавай всё сразу, начни с одного-двух.`;
